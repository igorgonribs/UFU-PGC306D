...
def _to_uint8(img_float):
    """Clipa para [0,1] e converte p/ uint8 [0,255]."""
    img_float = np.clip(img_float, 0.0, 1.0)
    return (img_float * 255.0).astype(np.uint8)

def _ensure_float01(img):
    """Converte uint8 [0,255] -> float [0,1]."""
    if img.dtype == np.uint8:
        return img.astype(np.float32) / 255.0
    # já está em float
    # se vier em 0..255, normaliza
    if img.max() > 1.0:
        return (img / 255.0).astype(np.float32)
    return img.astype(np.float32)

def random_augment_plate(img,
                         max_rot_deg=30,
                         ...
                         blur_sigma_max=0.6):
    """
    Aumenta a imagem respeitando restrições:
      - rotação |θ| ≤ max_rot_deg (sem flip!)
      - pan/translate ≤ max_translate_ratio da dimensão
      - escala leve (zoom-in/out)
    ...
    # ---- Afim: escala + rotação + translação (sem flips) ----
    # rotação
    ...
    # translação: ≤ 30% de cada dimensão
    max_tx = max_translate_ratio * w
    max_ty = max_translate_ratio * h
    tx = rng.uniform(-max_tx, max_tx)
    ty = rng.uniform(-max_ty, max_ty)

    # Monta a matriz afim (centra em torno do meio, aplica, volta)
    center = (w / 2.0, h / 2.0)
    # Construímos em 3 etapas: translate(-center) -> rotate+scale -> translate(center) -> translate(tx,ty)
    ...
    M = (t1 + r) + t2  # composição

    warped = warp(
        imgf,
        inverse_map=M.inverse,
        order=1,           # bilinear
        mode="edge",       # evita "espelhamento" explícito; estende borda
        preserve_range=True
    )

    # ---- Jitter de brilho/contraste ----
    # contraste: y = (x - 0.5)*c + 0.5 ; brilho: y = x + b
    c = 1.0 + rng.uniform(-contrast_jitter, contrast_jitter)
    b = rng.uniform(-brightness_jitter, brightness_jitter)

    warped = (warped - 0.5) * c + 0.5
    warped = warped + b
    warped = np.clip(warped, 0.0, 1.0)

    # ---- Ruído gaussiano leve ----
    if noise_std_max > 0:
        std = rng.uniform(0.0, noise_std_max)  # até 0.02 (~5/255)
        if std > 1e-6:
            noise = rng.normal(0.0, std, size=warped.shape).astype(np.float32)
            warped = np.clip(warped + noise, 0.0, 1.0)

    # ---- Desfoque leve ----
    if blur_sigma_max > 0:
        sigma = rng.uniform(0.0, blur_sigma_max)
        if sigma > 1e-6:
            # multicanal: aplica por canal
...
def oversample_with_augmentation(X_train, y_train, random_seed=42):
    """
    Oversampling por classe até a contagem da classe majoritária usando augmentations.
    - Não espelha imagem.
    - Rotação limitada a ±30°.
    - Pan/translate ≤ 30% de W/H.
    Retorna listas X_train_oversampled, y_train_oversampled.
    """
    rng_local = np.random.default_rng(random_seed)

    df = pd.DataFrame({"Image": list(X_train), "ClassName": list(y_train)})
    ...
    for cls, n in counts.items():
        cls_df = df[df["ClassName"] == cls]
        imgs = list(cls_df["Image"])

        # Se já é majoritária, só acumula os originais
        new_imgs.extend(imgs)
        new_lbls.extend([cls] * len(imgs))

        # Quantos precisamos sintetizar
        need = max_count - n
        if need <= 0:
            continue

        # Vamos amostrar com reposição da própria classe e aplicar augment
        for _ in range(need):
            base_img = imgs[rng_local.integers(0, len(imgs))]
            aug_img = random_augment_plate(base_img)
            new_imgs.append(aug_img)
            new_lbls.append(cls)

    # Embaralha o resultado final
    idx = np.arange(len(new_imgs))
    rng_local.shuffle(idx)
    X_bal = [new_imgs[i] for i in idx]
    y_bal = [new_lbls[i] for i in idx]
    return X_bal, y_bal

# ---------- Executar ----------
X_train_oversampled, y_train_oversampled = oversample_with_augmentation(X_train, y_train, random_seed=RANDOM_SEED)

print("Antes:")
print(pd.Series(y_train).value_counts().sort_index())
print("\nDepois (oversampling com augmentation):")
print(pd.Series(y_train_oversampled).value_counts().sort_index())
...
tensor_x_treino = torch.from_numpy(np.array(list(X_train_oversampled), dtype=np.uint8)).permute(0, 3, 1, 2).to(torch.float32) / 255.0
...
tensor_y_treino = torch.from_numpy(np.array([class_to_id[label] for label in y_train_oversampled])).to(torch.long)
...
trials_df, best_hist_df = run_hparam_search(n_trials=25, n_epochs=40, output_prefix="experimento_03_")
trials_df.head()
...
plt.tight_layout()
filename=GRAPH_DIR+'/experimento_03_curvas_loss.png'
...
plt.tight_layout()
filename=GRAPH_DIR+'/experimento_03_curvas_acuracia.png'
plt.savefig(filename, dpi=150)
plt.show()
...
output_prefix = "experimento_03_" # Certifique-se que este prefixo corresponde ao usado em run_hparam_search
